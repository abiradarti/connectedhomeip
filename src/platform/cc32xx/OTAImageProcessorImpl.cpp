...
{
    if (nullptr == mNvsHandle) //replace with CC32xx implementation
    {
...
{
    NVS_Params nvsParams; //replace with CC32xx implementation 
    NVS_Handle handle;    //replace with CC32xx implementation
    ...
    NVS_Params_init(&nvsParams); //replace with CC32xx implementation
    handle = NVS_open(CONFIG_NVSEXTERNAL, &nvsParams); //replace with CC32xx implementation
    if (NULL == handle) //replace with CC32xx implementation
    {
        imageProcessor->mDownloader->OnPreparedForDownload(CHIP_ERROR_OPEN_FAILED);
        return;
    }

    if (!eraseExtFlashMetaHeader(handle)) //replace with CC32xx implementation
    {
        NVS_close(handle); //replace with CC32xx implementation
        imageProcessor->mDownloader->OnPreparedForDownload(CHIP_ERROR_WRITE_FAILED);
        return;
    }

    imageProcessor->mNvsHandle = handle; //replace with CC32xx implementation
...
void OTAImageProcessorImpl::HandleFinalize(intptr_t context)
{
    ...
    //replace with CC32xx implementation
...
{
    ExtImageInfo_t header; //replace with CC32xx implementation
    auto * imageProcessor = reinterpret_cast<OTAImageProcessorImpl *>(context);
    if (imageProcessor == nullptr)
    {
        return;
    ...
    if (!readExtFlashMetaHeader(imageProcessor->mNvsHandle, &header)) //replace with CC32xx implementation
    {
        return;
    }
    header.fixedHdr.imgCpStat = NEED_COPY; //replace with CC32xx implementation

    writeExtFlashMetaHeader(imageProcessor->mNvsHandle, &header); //replace with CC32xx implementation

    // reset SoC to kick BIM
    SysCtrlSystemReset(); //replace with CC32xx implementation
}
...
void OTAImageProcessorImpl::HandleAbort(intptr_t context)
{
    ...
    if (!eraseExtFlashMetaHeader(imageProcessor->mNvsHandle)) //replace with CC32xx implementation
    {
        imageProcessor->mDownloader->OnPreparedForDownload(CHIP_ERROR_WRITE_FAILED);
    }

    NVS_close(imageProcessor->mNvsHandle); //replace with CC32xx implementation
    imageProcessor->ReleaseBlock();
}
...
void OTAImageProcessorImpl::HandleProcessBlock(intptr_t context)
{
    ...
    else if (imageProcessor->mDownloader == nullptr) //replace with CC32xx implementation
    {
        ChipLogError(SoftwareUpdate, "mDownloader is null");
        return;
    }
    ...
    if (!writeExtFlashImgPages(imageProcessor->mNvsHandle, imageProcessor->mParams.downloadedBytes, imageProcessor->mBlock)) //replace with CC32xx implementation
    {
